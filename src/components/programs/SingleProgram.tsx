...
const SingleProgram: React.FC<program> = (program) => {
    return (
        <div className='h-auto bg-white dark:bg-[#232323] border border-black/10 dark:border-white/10 rounded-md w-auto shadow-md shadow-slate-900/5 p-4 md:p-6 flex flex-col items-start justify-center gap-1 md:gap-2'>
            <div className='flex gap-2 items-center justify-start w-full'>
                <Image src={program.thumbnail} alt={program.title} className='w-16 h-16 rounded-full object-fill drop-shadow-sm' />
                <div className="flex flex-col items-start justify-center gap-1 w-full">
                    <h4 className='text-sm md:text-[16px] font-normal w-[90%]'>{program.title}</h4>
                    <p className='text-sm text-muted-foreground'>{program.duration}</p>
                </div>
            </div>
            <div className="flex justify-between w-full items-center gap-2">
                <p className='text-sm text-muted-foreground'>{program.description}</p>
                <svg xmlns="http://www.w3.org/2000/svg" width="38" height="38" className='w-12 h-12' viewBox="0 0 46 46" fill="none">
